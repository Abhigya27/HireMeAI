...
def get_readme(owner: str, repo: str, branch: str = "main") -> tuple[str, str] | None:
    ...
    try:
        response = httpx.get(url, headers=_headers(), params={"ref": branch}, timeout=15.0)
        ...
        logger.warning(f"[github_client] get_readme failed for {owner}/{repo}: {e}")
        return None
...
def get_file(owner: str, repo: str, path: str, branch: str = "main") -> tuple[str, str] | None:
    ...
    try:
        ...
        logger.warning(f"[github_client] get_file failed for {owner}/{repo}/{path}: {e}")
        return None
...
def list_files(owner: str, repo: str, branch: str = "main") -> list[str]:
    ...
    try:
        response = httpx.get(url, headers=_headers(), params={"recursive": "1"}, timeout=15.0)
        response.raise_for_status()
        tree = response.json().get("tree", [])
        return [item["path"] for item in tree if item.get("type") == "blob"]
    except httpx.HTTPError as e:
        logger.warning(f"[github_client] list_files failed for {owner}/{repo}: {e}")
        return []
